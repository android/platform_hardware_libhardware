...
keymaster_error_t KeystoreEnforcement::authorizeAuthTimeout(
    ...
    if (difftime(currentTime, lastAuthTime) > required_time) {
        return KM_ERROR_OK;
    } else {
...
keymaster_error_t KeystoreEnforcement::authorizeRescope(km_id_t keyid,
    ...
    int rescopeAuthIndex = oldAuthSet->find(KM_TAG_RESCOPE_AUTH_TIMEOUT);
    if (rescopeAuthIndex >= 0) {
        keymaster_key_param_t rescopeAuthParam = (*oldAuthSet)[rescopeAuthIndex];
        keymaster_error_t authError = authorizeAuthTimeout(rescopeAuthParam, currentTime);
        if (authError != KM_ERROR_OK) {
            return authError;
        }
    }
