...
keymaster_error_t KeystoreEnforcement::AuthorizeOperation(const km_id_t keyid,
        const AuthorizationSet *auth_set, const uid_t uid, const keymaster_blob_t appid) {
    ...
    int purpose_index = auth_set->find(KM_TAG_PURPOSE);
    if (purpose_index < 0) {
        return_error = KM_ERROR_UNSUPPORTED_PURPOSE;
        goto clean_up;
    }
    purpose = static_cast<keymaster_purpose_t> ((*auth_set)[purpose_index].enumerated);
    ...
    for (unsigned int i = 0; return_error == KM_ERROR_OK && i < auth_set->size(); i++) {
        ...
        return_error = AuthorizeParameter(auth_set, keyid, param, purpose, uid, appid);
    }
...
keymaster_error_t KeystoreEnforcement::AuthorizeParameter(const AuthorizationSet *auth_set,
        const km_id_t keyid, const keymaster_key_param_t param, const keymaster_purpose_t purpose,
    ...
    switch (tag) {
        ...
        case KM_TAG_ORIGINATION_EXPIRE_DATETIME:
            return_error = authorize_origination_expire_time(param, current_time,
                    purpose);
            break;
        ...
        case KM_TAG_ALL_APPLICATIONS:
            return_error = authorize_all_apps(auth_set);
            break;
        case KM_TAG_APPLICATION_ID:
            return_error = authorize_auth_id(auth_set, param, appid);
            break;
        default:
            return_error = KM_ERROR_OK;
            break;
...
keymaster_error_t KeystoreEnforcement::authorize_active_time( const keymaster_key_param_t param,
...
keymaster_error_t KeystoreEnforcement::authorize_usage_expire_time( const keymaster_key_param_t param,
...
keymaster_error_t KeystoreEnforcement::authorize_origination_expire_time(
        const keymaster_key_param_t param, const time_t current_time,
...
keymaster_error_t KeystoreEnforcement::authorize_min_time_between_ops(const keymaster_key_param_t param,
...
keymaster_error_t KeystoreEnforcement::authorize_single_use_per_boot(const keymaster_key_param_t param,
        const km_id_t keyid) {
...
keymaster_error_t KeystoreEnforcement::authorize_user_id(const keymaster_key_param_t param,
        const uid_t uid) {
...
keymaster_error_t KeystoreEnforcement::authorize_auth_timeout(const keymaster_key_param_t param,
        const time_t current_time) {
...
keymaster_error_t KeystoreEnforcement::authorize_auth_id(const AuthorizationSet *auth_set,
...
keymaster_error_t KeystoreEnforcement::authorize_all_apps(const AuthorizationSet *auth_set) {
    if (auth_set->find(KM_TAG_APPLICATION_ID) != -1) {
        return KM_ERROR_INVALID_USER_ID;
    }
    return KM_ERROR_OK;
}

keymaster_error_t KeystoreEnforcement::authorize_purpose(const keymaster_key_param_t purpose_param,
        const AuthorizationSet *auth_set) {
    int purpose_index;
    keymaster_purpose_t purpose, test_purpose;

    purpose = static_cast<keymaster_purpose_t> (purpose_param.enumerated);

    purpose_index = auth_set->find(KM_TAG_PURPOSE);
    for (; purpose_index >= 0; purpose_index = auth_set->find(KM_TAG_PURPOSE)) {
...
bool KeystoreEnforcement::valid_rescope_del(const AuthorizationSet *auth_set, const keymaster_tag_t tag) {
...
bool KeystoreEnforcement::valid_rescope_add(const AuthorizationSet *auth_set, const keymaster_tag_t tag) {
...
keymaster_error_t KeystoreEnforcement::AuthorizeRescope(const km_id_t keyid,
        const AuthorizationSet *old_auth_set, const AuthorizationSet *new_auth_set, const uid_t uid) {
    time_t current_time = get_current_time();
...
void KeystoreEnforcement::update_key_access_time(const km_id_t keyid) {
    uint32_t key_index = key_id_to_index(keyid);
    accessTimeMap.update_key_access_time(keyid, get_current_time());
}

uint32_t KeystoreEnforcement::key_id_to_index(const km_id_t keyid) {
    return keyid;
}

bool KeystoreEnforcement::km_param_compare(const keymaster_key_param_t param1,
...
time_t KeystoreEnforcement::get_current_time() {
    return time(NULL);
}

time_t KeystoreEnforcement::get_last_access_time(km_id_t keyid) {
    uint32_t key_index = key_id_to_index(keyid);
    return accessTimeMap.get_last_key_access_time(key_index);
}

uint32_t KeystoreEnforcement::get_user_id_from_uid(uid_t uid) {
    uint32_t userId = uid / MULTIUSER_APP_PER_USER_RANGE;
    return userId;
}

time_t KeystoreEnforcement::get_last_auth_time() {
    return last_auth_time;
}

void KeystoreEnforcement::update_user_authentication_time() {
    get_key_lock();
    last_auth_time = get_current_time();
    release_key_lock();
}
bool KeystoreEnforcement::is_valid_purpose(keymaster_purpose_t purpose) {
    bool ret_val;
...
int KeystoreEnforcement::get_key_lock() {
#ifdef ANDROID_UNTRUSTED_ZONE
...
int KeystoreEnforcement::release_key_lock() {
#ifdef ANDROID_UNTRUSTED_ZONE
...
KeystoreEnforcement::AccessTimeMap::AccessTimeMap() {
}

List<access_time_struct>::iterator KeystoreEnforcement::AccessTimeMap::find(uint32_t key_index) {
...
void KeystoreEnforcement::AccessTimeMap::update_key_access_time(uint32_t key_index,
        time_t current_time) {
    List<access_time_struct>::iterator posn;

    posn = find(key_index);
    if (posn != last_access_list.end()) {
        (*posn).atime = current_time;
        //posn = last_access_list.erase(posn);
    } else {
        access_time_struct ac;
        ac.keyid = key_index;
        ac.atime = current_time;
        last_access_list.push_front(ac);
    }
}

time_t KeystoreEnforcement::AccessTimeMap::get_last_key_access_time(uint32_t key_index) {
    List<access_time_struct>::iterator posn;
