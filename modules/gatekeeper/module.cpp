...
#define LOG_TAG "SoftwareGateKeeper"
#include <log/log.h>
...
using software::SoftGateKeeperDevice;

struct software_gatekeeper_device {
    gatekeeper_device device;
    SoftGateKeeperDevice *s_gatekeeper;
};

static software_gatekeeper_device s_device;
...
static int enroll(const struct gatekeeper_device *dev __unused, uint32_t uid,
    ...
    SoftGateKeeperDevice *s_gatekeeper = ((software_gatekeeper_device*)(dev))->s_gatekeeper;
...
static int software_gatekeeper_open(const hw_module_t *module, const char *name,
        hw_device_t **device) {
