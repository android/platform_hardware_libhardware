...
EvdevSensor::EvdevSensor(
    ...
    initFeatureValueFromEvdevDeviceInfo(&mFeatureValue, info, axisInfo);
    ...
    switch (usage) {
        case ACCELEROMETER_3D:
            ...
            // Android unit m/s^2
            // 1g = 9.81 m/s^2
            mFeatureValue.typeString = SENSOR_STRING_TYPE_ACCELEROMETER;
            mFeatureValue.type = SENSOR_TYPE_ACCELEROMETER;
            mFeatureValue.isWakeUp = false;
        ...
        case GYROMETER_3D:
            ...
            mFeatureValue.typeString = SENSOR_STRING_TYPE_GYROSCOPE;
            mFeatureValue.type = SENSOR_TYPE_GYROSCOPE;
            mFeatureValue.isWakeUp = false;
...
void EvdevSensor::processTriAxisUsage(uint16_t axis0, uint16_t axis1, uint16_t axis2,
    ...
    // scale resolution
    mFeatureValue.resolution = mFeatureValue.resolution * defaultScaling;

    EventTranslateRecord record = {
        .minValue = mFeatureValue.minValue,
        .maxValue = mFeatureValue.maxValue,
        .type = TYPE_FLOAT
    };
    ...
    record.a = mFeatureValue.resolution;
    record.b = 0;
...
void EvdevSensor::initFeatureValueFromEvdevDeviceInfo(
    ...
    // axisInfo.resolution specifies the number of steps per unit, e.g. 8192 steps per G.
    // featureValue->resolution expects the normalized factor, e.g. to normalize to 1G,
    // axisInfo.resolution is inverted.
    ...
    featureValue->reportModeFlag = SENSOR_FLAG_CONTINUOUS_MODE;
...
bool EvdevSensor::validateFeatureValueAndBuildSensor() {
    if (mFeatureValue.name.empty() || mFeatureValue.vendor.empty()
            || mFeatureValue.typeString.empty() || mFeatureValue.type <= 0
            || mFeatureValue.maxRange <= 0 || mFeatureValue.resolution <= 0) {
        return false;
    }

    if (mFeatureValue.maxValue < mFeatureValue.minValue) {
        return false;
    }

    switch (mFeatureValue.reportModeFlag) {
        case SENSOR_FLAG_CONTINUOUS_MODE:
        case SENSOR_FLAG_ON_CHANGE_MODE:
            if (mFeatureValue.minDelay < 0) {
                return false;
            }
            if (mFeatureValue.maxDelay != 0 && mFeatureValue.maxDelay < mFeatureValue.minDelay) {
                return false;
            }
            break;
        case SENSOR_FLAG_ONE_SHOT_MODE:
            if (mFeatureValue.minDelay != -1 && mFeatureValue.maxDelay != 0) {
                return false;
            }
            break;
    ...
    if (mFeatureValue.fifoMaxSize != 0
            && mFeatureValue.fifoMaxSize < mFeatureValue.fifoSize) {
        return false;
    }

    // initialize uuid field, use name, vendor and uniqueId
    if (mFeatureValue.name.size() >= 4
            && mFeatureValue.vendor.size() >= 4
            && mFeatureValue.typeString.size() >= 4
            && mFeatureValue.uniqueId.size() >= 4) {
        uint32_t tmp[4], h;
        std::hash<std::string> stringHash;
        h = stringHash(mFeatureValue.uniqueId);
        tmp[0] = stringHash(mFeatureValue.name) ^ h;
        tmp[1] = stringHash(mFeatureValue.vendor) ^ h;
        tmp[2] = stringHash(mFeatureValue.typeString) ^ h;
        tmp[3] = tmp[0] ^ tmp[1] ^ tmp[2];
        memcpy(mFeatureValue.uuid, tmp, sizeof(mFeatureValue.uuid));
    }

    mSensor = (sensor_t) {
        mFeatureValue.name.c_str(),                // name
        mFeatureValue.vendor.c_str(),              // vendor
        mFeatureValue.version,                     // version
        -1,                                        // handle, dummy number here
        mFeatureValue.type,
        mFeatureValue.maxRange,                    // maxRange
        mFeatureValue.resolution,                  // resolution
        mFeatureValue.power,                       // power
        mFeatureValue.minDelay,                    // minDelay
        (uint32_t)mFeatureValue.fifoSize,          // fifoReservedEventCount
        (uint32_t)mFeatureValue.fifoMaxSize,       // fifoMaxEventCount
        mFeatureValue.typeString.c_str(),          // type string
        mFeatureValue.permission.c_str(),          // requiredPermission
        (long)mFeatureValue.maxDelay,              // maxDelay
        mFeatureValue.reportModeFlag | (mFeatureValue.isWakeUp ? 1 : 0),
        { NULL, NULL }
    };
    return true;
}
...
void EvdevSensor::getUuid(uint8_t* uuid) const {
    memcpy(uuid, mFeatureValue.uuid, sizeof(mFeatureValue.uuid));
}
...
std::string EvdevSensor::dump() const {
    ...
    ss << "Feature Values " << std::endl
          << "  name: " << mFeatureValue.name << std::endl
          << "  vendor: " << mFeatureValue.vendor << std::endl
          << "  permission: " << mFeatureValue.permission << std::endl
          << "  typeString: " << mFeatureValue.typeString << std::endl
          << "  type: " << mFeatureValue.type << std::endl
          << "  maxRange: " << mFeatureValue.maxRange << std::endl
          << "  resolution: " << mFeatureValue.resolution << std::endl
          << "  power: " << mFeatureValue.power << std::endl
          << "  minDelay: " << mFeatureValue.minDelay << std::endl
          << "  maxDelay: " << mFeatureValue.maxDelay << std::endl
          << "  fifoSize: " << mFeatureValue.fifoSize << std::endl
          << "  fifoMaxSize: " << mFeatureValue.fifoMaxSize << std::endl
          << "  reportModeFlag: " << mFeatureValue.reportModeFlag << std::endl
          << "  isWakeUp: " << (mFeatureValue.isWakeUp ? "true" : "false") << std::endl
          << "  uniqueId: " << mFeatureValue.uniqueId << std::endl
          << "  uuid: ";

    ss << std::hex << std::setfill('0');
    for (auto d : mFeatureValue.uuid) {
