...
EvdevDevice::EvdevDevice(
    ...
    if (!populateDeviceInfo()) {
        ALOGE("Error obtaining Evdev device information or not a sensor device");
        return;
    }
...
bool EvdevDevice::getAbsoluteAxisInfo(uint8_t axis, input_absinfo* outAxisInfo) const {
    ...
    memset(outAxisInfo, 0, sizeof(input_absinfo));
...
int EvdevDevice::hasEvent(uint16_t timeoutMs)
{
    ...
    int result = ::select(nfds, &fd, nullptr, nullptr, &tv);
