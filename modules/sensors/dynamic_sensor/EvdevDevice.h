...
/*
 * Evdev (event input device) sensors send its sensor data using specific
 * event types (e.g. EV_ABS) and event codes (e.g. ABS_X, ABS_Y, ABS_Z).
 * For example, evdev accelerometers send its X axis acceleration using
 * event code ABS_X, and evdev gyroscopes send its X axis rotation using
 * event code ABS_RX.
 *
 * Please refer to bionic/libc/kernel/uapi/linux/input-event-codes.h for all
 * event types and codes.
 *
 * For more information about evdev devices, please refer to
...
public:
    ...
    struct EvdevDeviceInfo {
        ...
        uint16_t product;
        uint16_t version;
        uint8_t propBitmask[(INPUT_PROP_MAX / 8) + 1];
        uint8_t absBitmask[(ABS_MAX / 8) + 1];
    };
