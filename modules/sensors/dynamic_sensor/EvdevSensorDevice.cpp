                    ...
                    case ACCELERATION_Z_AXIS:
                        {
                            ...
                            if (s == mSensors.end()) {
                                ALOGW("Input of unknown input event code %u received", inputEvent.code);
                                continue;
                            }
...
bool EvdevSensorDevice::getTriAbsoluteAxisInfo(input_absinfo* outAxisInfo,
    ...
    if (axis2Info.minimum >= axis2Info.maximum
            || axis2Info.minimum != outAxisInfo->minimum
            || axis2Info.maximum != outAxisInfo->maximum) {
        ALOGE("All 3 axis should have same min and max value and min must < max");
        return false;
    }
    if (axis2Info.resolution != outAxisInfo->resolution) {
    ...
    if (axis3Info.minimum >= axis3Info.maximum
            || axis3Info.minimum != outAxisInfo->minimum
            || axis3Info.maximum != outAxisInfo->maximum) {
        ALOGE("All 3 axis should have same min and max value and min must < max");
        return false;
    }
    if (axis3Info.resolution != outAxisInfo->resolution) {
        ALOGE("All 3 axis should have same resolution");
        return false;
    }
