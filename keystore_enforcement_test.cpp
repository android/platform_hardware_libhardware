...
TEST_F(KeystoreBaseTest, TEST_VALID_KEY_PERIOD_NO_TAGS) {
    ...
    keymaster_error_t kme =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &single_auth_set, uid, def_app_id);
...
TEST_F(KeystoreBaseTest, TEST_INVALID_ACTIVE_TIME) {
    ...
    keymaster_error_t kme_invalid_time =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set, uid, def_app_id);
...
TEST_F(KeystoreBaseTest, TEST_VALID_ACTIVE_TIME) {
    ...
    keymaster_error_t kme_valid_time =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set, uid, def_app_id);
...
TEST_F(KeystoreBaseTest, TEST_INVALID_USAGE_EXPIRE_TIME) {
    ...
    keymaster_error_t kme_invalid_origination =
        kse.AuthorizeOperation(KM_PURPOSE_VERIFY, key_id, &auth_set, uid, def_app_id);
    ASSERT_EQ(KM_ERROR_KEY_EXPIRED, kme_invalid_origination);
}
...
TEST_F(KeystoreBaseTest, TEST_VALID_SINGLE_USE_ACCESSES) {
    ...
    keymaster_error_t kme1 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set, uid, def_app_id);
    keymaster_error_t kme2 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set, uid, def_app_id);
...
TEST_F(RescopeBaseTest, TEST_RESCOPE_DEL_SUB) {
    ...
    KeystoreEnforcement kse;
    AuthorizationSet auth_set(params1, 6);
    AuthorizationSet auth_set2(params2, 3);

    ASSERT_TRUE(kse.valid_rescope_del(&auth_set, KM_TAG_USER_ID));
    ASSERT_FALSE(kse.valid_rescope_del(&auth_set, KM_TAG_ALL_USERS));
    ASSERT_FALSE(kse.valid_rescope_del(&auth_set2, KM_TAG_PURPOSE));
    ASSERT_FALSE(kse.valid_rescope_del(&auth_set2, KM_TAG_USER_ID));
}
...
TEST_F(RescopeBaseTest, TEST_RESCOPE_ADD_SUB) {
    ...
    ASSERT_TRUE(kse.valid_rescope_add(&auth_set, KM_TAG_SINGLE_USE_PER_BOOT));
    ASSERT_TRUE(kse.valid_rescope_add(&auth_set, KM_TAG_USAGE_EXPIRE_DATETIME));
    ASSERT_FALSE(kse.valid_rescope_add(&auth_set, KM_TAG_USER_ID));

    ASSERT_FALSE(kse.valid_rescope_add(&auth_set2, KM_TAG_RESCOPE_AUTH_TIMEOUT));
    ASSERT_FALSE(kse.valid_rescope_add(&auth_set2, KM_TAG_SINGLE_USE_PER_BOOT));
}
...
TEST_F(KeystoreBaseTest, TEST_NO_RESCOPES) {
    ...
    ASSERT_EQ(KM_ERROR_OK, kse.AuthorizeRescope(1, &auth_set1, &auth_set2, 1));
}
...
TEST_F(RescopeBaseTest, TEST_VALID_RESCOPE_ADD) {
    ...
    ASSERT_EQ(KM_ERROR_OK, kse.AuthorizeRescope(1, &auth_set1, &auth_set2, 1));
    ASSERT_EQ(KM_ERROR_OK, kse.AuthorizeRescope(1, &auth_set1, &auth_set3, 1));
}
...
TEST_F(RescopeBaseTest, TEST_VALID_RESCOPE_ADD_DEL) {
    ...
    ASSERT_EQ(KM_ERROR_OK, kse.AuthorizeRescope(1, &auth_set1, &auth_set2, 1));
    ASSERT_EQ(KM_ERROR_OK, kse.AuthorizeRescope(1, &auth_set1, &auth_set3, 1));
    ASSERT_EQ(KM_ERROR_OK, kse.AuthorizeRescope(1, &auth_set1, &auth_set4, 1));
}
...
TEST_F(RescopeBaseTest, TEST_INVALID_RESCOPE_ADD) {
    ...
    ASSERT_EQ(KM_ERROR_INVALID_RESCOPING, kse.AuthorizeRescope(1, &auth_set1, &auth_set2, 1));
}
...
TEST_F(RescopeBaseTest, TEST_INVALID_RESCOPE_ADD_DEL) {
    ...
    ASSERT_EQ(KM_ERROR_INVALID_RESCOPING, kse.AuthorizeRescope(1, &auth_set1, &auth_set2, 1));
    ASSERT_EQ(KM_ERROR_INVALID_RESCOPING, kse.AuthorizeRescope(1, &auth_set1, &auth_set3, 1));
    ASSERT_EQ(KM_ERROR_INVALID_RESCOPING, kse.AuthorizeRescope(1, &auth_set1, &auth_set4, 1));
}
...
TEST_F(ComparisonBaseTest, TEST_BOOLEAN_COMPARISON) {
    ...
    ASSERT_TRUE(KeystoreEnforcement::km_param_compare(b1, b2));
}
...
TEST_F(KeystoreBaseTest, TEST_USER_ID) {
    ...
    uint32_t validuid1 = valid_user_id * KeystoreEnforcement::MULTIUSER_APP_PER_USER_RANGE +
                         (appId1 % KeystoreEnforcement::MULTIUSER_APP_PER_USER_RANGE);
    uint32_t validuid2 = valid_user_id * KeystoreEnforcement::MULTIUSER_APP_PER_USER_RANGE +
                         (appId1 % KeystoreEnforcement::MULTIUSER_APP_PER_USER_RANGE);

    uint32_t invaliduid1 = invalid_user_id1 * KeystoreEnforcement::MULTIUSER_APP_PER_USER_RANGE +
                           (appId1 % KeystoreEnforcement::MULTIUSER_APP_PER_USER_RANGE);
    uint32_t invaliduid2 = invalid_user_id1 * KeystoreEnforcement::MULTIUSER_APP_PER_USER_RANGE +
                           (appId2 % KeystoreEnforcement::MULTIUSER_APP_PER_USER_RANGE);
    uint32_t invaliduid3 = invalid_user_id2 * KeystoreEnforcement::MULTIUSER_APP_PER_USER_RANGE +
                           (appId1 % KeystoreEnforcement::MULTIUSER_APP_PER_USER_RANGE);
    uint32_t invaliduid4 = invalid_user_id2 * KeystoreEnforcement::MULTIUSER_APP_PER_USER_RANGE +
                           (appId2 % KeystoreEnforcement::MULTIUSER_APP_PER_USER_RANGE);
    ...
    keymaster_error_t valid_kme1 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set, validuid1, def_app_id);
    keymaster_error_t valid_kme2 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set, validuid2, def_app_id);

    keymaster_error_t invalid_kme1 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set, invaliduid1, def_app_id);
    keymaster_error_t invalid_kme2 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set, invaliduid2, def_app_id);
    keymaster_error_t invalid_kme3 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set, invaliduid3, def_app_id);
    keymaster_error_t invalid_kme4 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set, invaliduid4, def_app_id);

    ASSERT_EQ(KM_ERROR_OK, valid_kme1);
    ASSERT_EQ(KM_ERROR_OK, valid_kme2);

    ASSERT_EQ(KM_ERROR_INVALID_USER_ID, invalid_kme1);
    ASSERT_EQ(KM_ERROR_INVALID_USER_ID, invalid_kme2);
    ASSERT_EQ(KM_ERROR_INVALID_USER_ID, invalid_kme3);
    ASSERT_EQ(KM_ERROR_INVALID_USER_ID, invalid_kme4);
}
...
TEST_F(KeystoreBaseTest, TEST_APP_ID) {
    ...
    keymaster_error_t valid_kme1 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set1, uid, validBlob1);
    keymaster_error_t valid_kme2 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set2, uid, validBlob2);
    keymaster_error_t valid_kme3 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set4, uid, validBlob2);

    keymaster_error_t invalid_kme1 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set1, uid, invalidBlob1);
    keymaster_error_t invalid_kme2 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set1, uid, invalidBlob2);
    keymaster_error_t invalid_kme3 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set2, uid, invalidBlob1);
    keymaster_error_t invalid_kme4 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set2, uid, invalidBlob2);
    keymaster_error_t invalid_kme5 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set3, uid, validBlob2);

    ASSERT_EQ(KM_ERROR_OK, valid_kme1);
    ASSERT_EQ(KM_ERROR_OK, valid_kme2);
    ASSERT_EQ(KM_ERROR_OK, valid_kme3);

    ASSERT_NE(KM_ERROR_OK, invalid_kme1);
    ASSERT_NE(KM_ERROR_OK, invalid_kme2);
    ASSERT_NE(KM_ERROR_OK, invalid_kme3);
    ASSERT_NE(KM_ERROR_OK, invalid_kme4);
    ASSERT_NE(KM_ERROR_OK, invalid_kme5);
}
...
TEST_F(KeystoreBaseTest, TEST_INVALID_RESCOPE_AUTH_TIMEOUT) {
    ...
    kse.update_user_authentication_time();
    keymaster_error_t kme = kse.AuthorizeRescope(key_id, &auth_set1, &auth_set2, uid);
    ASSERT_EQ(KM_ERROR_KEY_USER_NOT_AUTHENTICATED, kme);
}
...
TEST_F(KeystoreBaseTest, TEST_INVALID_PURPOSE) {
    ...
    keymaster_error_t kme1 =
        kse.AuthorizeOperation(invalidPurpose1, key_id, &auth_set1, uid, def_app_id);
    keymaster_error_t kme2 =
        kse.AuthorizeOperation(invalidPurpose2, key_id, &auth_set2, uid, def_app_id);
    keymaster_error_t kme3 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set2, uid, def_app_id);
...
TEST_F(KeystoreBaseTest, TEST_INCOMPATIBLE_PURPOSE) {
    keymaster_key_param_t params[] = {
        keymaster_param_enum(KM_TAG_PURPOSE, KM_PURPOSE_VERIFY),
        keymaster_param_enum(KM_TAG_ALGORITHM, KM_ALGORITHM_RSA),
        keymaster_param_date(KM_TAG_ACTIVE_DATETIME, past_time),
    ...
    keymaster_error_t kme_invalid1 =
        kse.AuthorizeOperation(KM_PURPOSE_ENCRYPT, key_id, &auth_set, uid, def_app_id);
    keymaster_error_t kme_invalid2 =
        kse.AuthorizeOperation(KM_PURPOSE_DECRYPT, key_id, &auth_set, uid, def_app_id);

    keymaster_error_t kme_valid1 =
        kse.AuthorizeOperation(KM_PURPOSE_SIGN, key_id, &auth_set, uid, def_app_id);
    keymaster_error_t kme_valid2 =
        kse.AuthorizeOperation(KM_PURPOSE_VERIFY, key_id, &auth_set, uid, def_app_id);

    ASSERT_EQ(KM_ERROR_OK, kme_valid1);
    ASSERT_EQ(KM_ERROR_OK, kme_valid2);
    ASSERT_EQ(KM_ERROR_INCOMPATIBLE_PURPOSE, kme_invalid1);
    ASSERT_EQ(KM_ERROR_INCOMPATIBLE_PURPOSE, kme_invalid2);
}
