...
/** Represents the status of AGPS. */
typedef struct {
    /** set to sizeof(AGpsStatus_v4) */
    size_t                    size;

    AGpsType                  type;
    AGpsStatusValue           status;
    /** ipaddr - must be set to INADDR_NONE or a valid
        IPv4 address to achieve backward compatibility. */
    uint32_t                  ipaddr;
    /** addr - new HAL will use addr to provide IP address Info. */
    struct sockaddr_storage   addr;
} AGpsStatus_v4;

typedef AGpsStatus_v4 AGpsStatus;
...
typedef struct {
     ...
     * the name of the APN to be used for SUPL.
     */
    int  (*data_conn_open_with_apniptype)( const char* apn, ApnIpType apnIpType );
} AGpsInterface_v2;
