...
struct nvram_device {
    ...
    */
    nvram_result_t (*get_space_list)(const struct nvram_device* device, uint32_t max_list_size,
    ...
    */
    nvram_result_t (*get_space_controls)(const struct nvram_device* device, uint32_t index,
                                         uint32_t max_list_size, nvram_control_t* control_list,
                                         uint32_t* list_size);

    /**
    * Outputs whether locks are enabled for the given space. When a lock
    * is enabled, the operation is disabled and any attempt will result
    * in NV_RESULT_OPERATION_DISABLED.
    *
    *   index - The space index.
    ...
    */
    nvram_result_t (*is_space_locked)(const struct nvram_device* device, uint32_t index,
                                      int* write_lock_enabled, int* read_lock_enabled);
    ...
    */
    nvram_result_t (*create_space)(const struct nvram_device* device, uint32_t index,
                                   uint64_t size_in_bytes, nvram_control_t* control_list,
                                   uint32_t list_size, uint8_t* authorization_value,
    ...
    */
    nvram_result_t (*delete_space)(const struct nvram_device* device, uint32_t index,
                                   uint8_t* authorization_value, uint32_t authorization_value_size);

    /**
    * Disables any further creation of spaces until the next full device
    * reset. Subsequent calls to NV_CreateSpace should return
    * NV_RESULT_OPERATION_DISABLED.
    */
    ...
    *                 will be set to 0xFF. If this is more than the size
    ...
    * written, all bytes read will be 0xFF.
    *
    *   index - The space index.
    ...
    */
    nvram_result_t (*read_space)(const struct nvram_device* device, uint32_t index,
                                 uint64_t num_bytes_to_read, uint8_t* authorization_value,
    ...
    *                              |authorization_value|.
    */
    nvram_result_t (*enable_write_lock)(const struct nvram_device* device, uint32_t index,
    ...
    *                              |authorization_value|.
    */
    nvram_result_t (*enable_read_lock)(const struct nvram_device* device, uint32_t index,
                                       uint8_t* authorization_value,
                                       uint32_t authorization_value_size);
};
...
static inline int nvram_open(const struct hw_module_t* module, nvram_device_t** device) {
    return module->methods->open(module, NVRAM_HARDWARE_DEVICE_ID, (struct hw_device_t**)device);
}
