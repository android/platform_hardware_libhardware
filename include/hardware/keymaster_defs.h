...
typedef enum {
    ...
    KM_TAG_MAX_BOOT_LEVEL = KM_UINT | 1010, /* Specifies a maximum boot level at which a key
                                               ...
                                               advanceBootLevel, passing it an integer value. If a key
                                               is created with a MAX_BOOT_LEVEL of x, and
                                               advanceBootLevel is called with a value greater than x,
                                               that key can then not be used again until the next
                                               boot. This is cryptographically enforced where
                                               Keymaster 4.0 or greater is present, and can be used to
                                               defend against persistence of attacks. */
} keymaster_tag_t;
...
typedef enum {
    ...
    KM_ERROR_BOOT_LEVEL_EXCEEDED = -82,
