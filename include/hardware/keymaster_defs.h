...
typedef enum {
    ...
    KM_TAG_USER_SECURE_ID = KM_LONG_REP | 502, /* Secure ID of authorized user(s).  Disallowed if
                                                  KM_TAG_ALL_USERS or KM_TAG_NO_AUTH_REQUIRED is
                                                  present. */
    KM_TAG_NO_AUTH_REQUIRED = KM_BOOL | 503,   /* If key is usable without authentication. */
    KM_TAG_USER_AUTH_ID = KM_INT_REP | 504,    /* ID of the authenticator to use (e.g. password,
                                                  fingerprint, etc.).  Disallowed if
