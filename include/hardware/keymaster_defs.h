...
typedef enum {
  KM_INVALID =
      0 << 28, /* Invalid type, used to designate a tag as uninitialized */
  KM_ENUM = 1 << 28,
...
typedef enum {
  ...
  KM_TAG_MAC_LENGTH =
      KM_INT | 6, /* MAC or AEAD authentication tag length in bits. */
  KM_TAG_PADDING = KM_ENUM | 7, /* keymaster_padding_t. */
  KM_TAG_RETURN_UNAUTHED =
      KM_BOOL | 8, /* Allow AEAD decryption to return plaintext before it has
  ...
  KM_TAG_RESCOPING_ADD =
      KM_ENUM_REP | 101, /* Tags authorized for addition via rescoping. */
  KM_TAG_RESCOPING_DEL =
      KM_ENUM_REP | 102, /* Tags authorized for removal via rescoping. */
  ...
  /* Note there are no EC-specific params.  Field size is defined by
     KM_TAG_KEY_SIZE, and the
     curve is chosen from NIST recommendations for field size */
  KM_TAG_ECIES_SINGLE_HASH_MODE = KM_BOOL | 204,

  /*
   * Tags that should be semantically enforced by hardware if possible and will
   * otherwise be
  ...
  KM_TAG_ORIGINATION_EXPIRE_DATETIME =
      KM_DATE | 401, /* Date when new "messages" should no
                        longer be created. */
  KM_TAG_USAGE_EXPIRE_DATETIME =
      KM_DATE | 402, /* Date when existing "messages" should no
                        longer be trusted. */
  KM_TAG_MIN_SECONDS_BETWEEN_OPS =
      KM_INT | 403, /* Minimum elapsed time between
                       cryptographic operations with the key. */
  KM_TAG_MAX_USES_PER_BOOT =
      KM_INT | 404, /* Number of times the key can be used per
                       boot. */
  ...
  KM_TAG_USER_SECURE_ID =
      KM_LONG_REP | 502, /* Secure ID of authorized user or authenticator(s).
                            Disallowed if KM_TAG_ALL_USERS or
                            KM_TAG_NO_AUTH_REQUIRED is present. */
  KM_TAG_NO_AUTH_REQUIRED =
      KM_BOOL | 503, /* If key is usable without authentication. */
  KM_TAG_USER_AUTH_TYPE =
      KM_ENUM | 504, /* Bitmask of authenticator types allowed when
  ...
  KM_TAG_AUTH_TIMEOUT =
      KM_INT | 505, /* Required freshness of user authentication for
  ...
  KM_TAG_ALL_APPLICATIONS =
      KM_BOOL | 600, /* If key is usable by all applications. */
  KM_TAG_APPLICATION_ID =
      KM_BYTES | 601, /* ID of authorized application. Disallowed if
  ...
  KM_TAG_APPLICATION_DATA =
      KM_BYTES | 700, /* Data provided by authorized application. */
  KM_TAG_CREATION_DATETIME = KM_DATE | 701, /* Key creation time */
  KM_TAG_ORIGIN = KM_ENUM | 702,            /* keymaster_key_origin_t. */
  KM_TAG_ROLLBACK_RESISTANT =
      KM_BOOL | 703, /* Whether key is rollback-resistant. */
  KM_TAG_ROOT_OF_TRUST =
      KM_BYTES | 704, /* Root of trust ID.  Empty array means usable by all
                         roots. */

  /* Tags used only to provide data to or receive data from operations */
  KM_TAG_ASSOCIATED_DATA =
      KM_BYTES | 1000, /* Used to provide associated data for AEAD modes. */
  KM_TAG_NONCE = KM_BYTES | 1001, /* Nonce or Initialization Vector */
  KM_TAG_CHUNK_LENGTH =
      KM_INT | 1002, /* AEAD mode chunk size, in bytes.  0 means no limit,
                        which requires KM_TAG_RETURN_UNAUTHED. */
  KM_TAG_AUTH_TOKEN =
      KM_BYTES | 1003, /* Authentication token that proves secure user
...
/**
 * Symmetric block cipher modes that may be provided by keymaster
 *implementations.  Those that must
 * be provided by all implementations are tagged as "required".  This type is
 *new in 0_4.
 *
 * KM_MODE_FIRST_UNAUTHENTICATED, KM_MODE_FIRST_AUTHENTICATED and
 *KM_MODE_FIRST_MAC are not modes,
...
typedef enum {
  ...
  KM_MODE_XTS = 7, /* Note: requires double-length keys */
                    ...
                    * over unauthenticated modes for all purposes.  One of KM_MODE_GCM and
                    * KM_MODE_OCB is
                    * required. */
...
/**
 * Digests that may be provided by keymaster implementations.  Those that must
 * be provided by all
 * implementations are tagged as "required".  Those that have been added since
 * version 0_2 of the
...
/**
 * The wire format of the elliptic curve points that may be used for
 * ECDSA/ECIES.
 */
typedef enum {
...
/**
 * The origin of a key (or pair), i.e. where it was generated.  Origin and can
 * be used together to
 * determine whether a key may have existed outside of secure hardware.  This
 * type is new in 0_4.
 */
typedef enum {
...
/**
 * Formats for key import and export.  At present, only asymmetric key
 * import/export is supported.
 * In the future this list will expand greatly to accommodate asymmetric key
 * import/export.
 */
typedef enum {
...
typedef enum {
  ...
  KM_ERROR_UNSUPPORTED_KEY_VERIFICATION_ALGORITHM =
      -20, /* For PKCS8 & PKCS12 */
  ...
  /* Additional error codes may be added by implementations, but implementers
   * should coordinate
...
inline keymaster_key_param_t keymaster_param_enum(keymaster_tag_t tag,
                                                  uint32_t value) {
  // assert(keymaster_tag_get_type(tag) == KM_ENUM ||
  // keymaster_tag_get_type(tag) == KM_ENUM_REP);
...
inline keymaster_key_param_t keymaster_param_int(keymaster_tag_t tag,
                                                 uint32_t value) {
  // assert(keymaster_tag_get_type(tag) == KM_INT || keymaster_tag_get_type(tag)
  // == KM_INT_REP);
  keymaster_key_param_t param;
  memset(&param, 0, sizeof(param));
  param.tag = tag;
...
inline keymaster_key_param_t keymaster_param_long(keymaster_tag_t tag,
                                                  uint64_t value) {
...
inline keymaster_key_param_t keymaster_param_blob(keymaster_tag_t tag,
                                                  const uint8_t* bytes,
                                                  size_t bytes_len) {
...
inline keymaster_key_param_t keymaster_param_date(keymaster_tag_t tag,
                                                  uint64_t value) {
...
inline int keymaster_param_compare(const keymaster_key_param_t* a,
  ...
  switch (keymaster_tag_get_type(a->tag)) {
    ...
    case KM_BYTES:
      ...
      if (a->blob.data_length != 0 && b->blob.data_length == 0) return -1;
      if (a->blob.data_length == 0 && b->blob.data_length == 0) return 0;
      if (a->blob.data_length == 0 && b->blob.data_length > 0) return 1;

      retval = memcmp(a->blob.data, b->blob.data,
                      a->blob.data_length < b->blob.data_length
                          ? a->blob.data_length
                          : b->blob.data_length);
      if (retval != 0)
        return retval;
        ...
        if (a->blob.data_length < b->blob.data_length) return -1;
        if (a->blob.data_length > b->blob.data_length) return 1;
      };
  }
...
inline void keymaster_free_param_values(keymaster_key_param_t* param,
                                        size_t param_count) {
  while (param_count-- > 0) {
...
inline void keymaster_free_characteristics(
