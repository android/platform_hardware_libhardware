...
struct keymaster2_device {
     ...
     *
     * \param[in] param_sets_count Length of \p params.
     *
     ...
     * KM_TAG_APPLICATION_ID and KM_TAG_APPLICATION_DATA are never returned.
     */
    ...
    /**
     * Derives a shared secret key from \p key, which must be an ECDH key, and the public key found
     * in /p other_key_certificate, an X.509 certificate containing a compatible EC public key.  The
     ...
     * other desired key characteristics.  The resulting key material is not returns directly, but
     * instead a new keymaster key is created and the associated blob returned in \p agreed_key.  If
     ...
     * set must include the same KM_TAG_KDF to specify how the raw agreed key bytes will be
     * transformed to produce the key material.
     *
     ...
     * value defined in <TODO:swillden -- insert link here> which contains the key description.
     *
     * \param[in] dev The keymaster device structure.
     *
     * \param[in] key The keymaster key for which the attestation certificate will be generated.
     *
     ...
     *
     * \param[in] full_chain If true, the returned certificate chain will be a full chain, rooted at
     * a Google CA certificate.  If false, the returned chain will contain only a single entry, the
     * certificate for \p key_to_attest.
     *
     * \param[out] cert_chain An array of DER-encoded X.509 certificates. The first will be the
     * certificate for \p key_to_attest.  If the chain contains more than one certificate it will be
     * the full chain.  The caller takes ownership of the returned chain.
     *
     * \param[out] cert_chain_length the length of \p cert_chain.
     */
    keymaster_error_t (*attest_key)(const keymaster2_device_t* dev,
                                    const keymaster_key_blob_t* key_to_attest,
                                    const keymaster_param_set_t* attest_params, bool full_chain,
                                    keymaster_blob_t** cert_chain, size_t* cert_chain_length);
     ...
     * \param[in] dev The keymaster device structure.
     *
     * \param[in] key The keymaster key to upgrade.
     *
     ...
     */
    keymaster_error_t (*upgrade_key)(const keymaster2_device_t* dev,
                                     const keymaster_key_blob_t* key_to_upgrade,
                                     const keymaster_param_set_t* upgrade_params,
