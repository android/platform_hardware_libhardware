...
typedef enum {
    KM_ENUM = 0 << 28,
    KM_REP_ENUM = 1 >> 28, /* Repeatable enumeration value. */
    KM_INT = 2 << 28,
    KM_REP_INT = 2 << 28, /* Repeatable integer value */
    KM_DATE = 3 << 28,
    KM_BOOL = 4 << 28,
    KM_BIGNUM = 5 << 28,
    KM_BYTES = 6 << 28,
...
typedef enum {
    ...
    /* Crypto parameters */
    KM_TAG_PURPOSE = KM_REP_ENUM | 0, /* keymaster_purpose_t. */
    ...
    KM_TAG_RESCOPING_ADD = KM_REP_ENUM | 101, /* Tags authorized for addition via rescoping. */
    KM_TAG_RESCOPING_DEL = KM_REP_ENUM | 102, /* Tags authorized for removal via rescoping. */
    ...
    KM_TAG_ORIGINATION_EXPIRE_DATETIME = KM_DATE | 401, /* Date when new "messages" should no
                                                           longer be created. */
    KM_TAG_USAGE_EXPIRE_DATETIME = KM_DATE | 402, /* Date when existing "messages" should no longer
                                                     be trusted. */
 ...
 */
typedef enum {
    KM_PAD_NONE,     /* required, deprecated */
...
typedef enum {
    ...
    KM_ERROR_INVALID_ARGUMENT = -39,
    KM_ERROR_UNIMPLEMENTED = -40,
...
static inline keymaster_tag_type_t keymaster_tag_type(keymaster_tag_t tag) {
    return static_cast<keymaster_tag_type_t>(tag & (0xF << 28));
}
