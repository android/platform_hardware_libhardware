...
struct keymaster_device {
    ...
    /**
     * \deprecated Generates a public and private key. The key-blob returned is
     * opaque and must subsequently provided for signing and verification.
     *
     * Returns: 0 on success or an error code less than 0.
     */
    ...
    /**
     * \deprecated Imports a public and private key pair. The imported keys will
     * be in PKCS#8 format with DER encoding (Java standard). The key-blob
     * returned is opaque and will be subsequently provided for signing and
     * verification.
     *
     * Returns: 0 on success or an error code less than 0.
     */
    ...
    /**
     * \deprecated Gets the public key part of a key pair. The public key must
     * be in X.509 format (Java standard) encoded byte array.
     *
    ...
    /**
     * \deprecated Deletes all keys in the hardware keystore. Used when keystore
     * is reset completely.
     *
     * This function is optional and should be set to NULL if it is not
     * implemented.
    ...
    /**
     * \deprecated Signs data using a key-blob generated before. This can use
     * either an asymmetric key or a secret key.
     *
     * Returns: 0 on success or an error code less than 0.
     */
    ...
    /**
     * \deprecated Verifies data signed with a key-blob. This can use either an
     * asymmetric key or a secret key.
     *
...
static inline int keymaster_open(const struct hw_module_t* module, keymaster_device_t** device) {
    int rc = module->methods->open(module, KEYSTORE_KEYMASTER, (struct hw_device_t**)device);
