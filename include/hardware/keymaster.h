...
typedef enum {
    KM_ENUM = 0 << 28,
    KM_ENUM_REP = 1 << 28, /* Repeatable enumeration value. */
    KM_INT = 2 << 28,
    KM_INT_REP = 3 << 28, /* Repeatable integer value */
    KM_LONG = 4 << 28,
    KM_DATE = 5 << 28,
    KM_BOOL = 6 << 28,
    KM_BIGNUM = 7 << 28,
...
typedef enum {
    ...
    /* Crypto parameters */
    KM_TAG_PURPOSE = KM_ENUM_REP | 0, /* keymaster_purpose_t. */
    KM_TAG_ALGORITHM = KM_ENUM | 1,   /* keymaster_algorithm_t. */
    KM_TAG_KEY_SIZE = KM_INT | 2,     /* Key size in bits. */
    KM_TAG_BLOCK_MODE = KM_ENUM | 3,  /* keymaster_block_mode_t. */
    KM_TAG_DIGEST = KM_ENUM | 4,      /* keymaster_digest_t. */
    KM_TAG_MAC_LENGTH = KM_INT | 5,   /* MAC length in bits. */
    KM_TAG_PADDING = KM_ENUM | 6,     /* keymaster_padding_t. */
    KM_TAG_CHUNK_LENGTH = KM_INT | 7, /* AEAD mode minimum decryption chunk size, in bytes. */
...
typedef struct {
    ...
    union {
        ...
        keymaster_blob_t bignum; /* KM_BIGNUM */
        keymaster_blob_t bytes;  /* KM_BYTES */
    };
...
struct keymaster_device {
     ...
     *
     * The caller assumes ownership of the allocated \p key_blob data and \p characteristics
     * objects. Use free_characteristics() to deallocate the latter.
     */
...
/* Convenience method for extracting tag type from tag */

static inline keymaster_tag_type_t keymaster_tag_get_type(keymaster_tag_t tag) {
    return (keymaster_tag_type_t)(tag & (0xF << 28));
}

static inline uint32_t keymaster_tag_mask_type(keymaster_tag_t tag) { return tag & 0x0FFFFFFF; }
...
static inline void keymaster_free_param_values(keymaster_key_param_t* param, size_t param_count) {
    while (param_count-- > 0) {
        ...
        case KM_BIGNUM:
            free((void*)param->bignum.data);
            break;
        case KM_BYTES:
            free((void*)param->bytes.data);
            break;
        default:
            // NOP
            break;
